...
def start_scan_helper(target_urls, scan_profile, scan_info_setup):
    ...
    """
    # set the scan info .commented by Lightning
    scan_info = ScanInfo()
    SCANS[get_new_scan_id()] = scan_info
    scan_info.w3af_core = w3af_core = w3afCore()
    scan_info.target_urls = target_urls
    # set the log output .commented by Lightning
    scan_info.output = RESTAPIOutput()

    # release the scan thread in the API SCAN thread .commented by Lightning
    scan_info_setup.set()

    # use the scan profile which input to create a temp file .commented by Lightning
